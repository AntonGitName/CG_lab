    ...
    , m_magIndex(0)
{
    ...
    for (int t = 1; t <= 8; ++t)
    {
        m_airplanes.push_back(std::shared_ptr<AirplaneObject>(new AirplaneObject(m_d3ddev, t)));
    }
    ...
    m_objects.push_back(std::shared_ptr<GObject>(new SphereObject(m_d3ddev, { 0.0f, -7.0f, 0.0f }, 7.0f)));
    m_objects.push_back(std::shared_ptr<GObject>(new Square(m_d3ddev, { 20.0f, -7.0f, 25.0f }, 7.0f)));
    for (int i = 0; i <= 8; ++i)
    {
        m_objects.push_back(std::shared_ptr<GObject>(new SphereTest(m_d3ddev, { 0.0f, -5.0f, 10.0f * (i + 1) }, 2 * (float)rand() / (float)RAND_MAX)));
    }
    for (auto airplane : m_airplanes)
    {
        m_objects.push_back(airplane);
    }
